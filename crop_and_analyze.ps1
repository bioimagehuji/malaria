...
$env:MALARIA_MAX_CROPS = 1  # Limit number of crops per series in crop.ijm. 0 is unlimited.
Write-Host "MALARIA_MAX_CROPS: '$env:MALARIA_MAX_CROPS'"
...
if (Test-Path $spreadsheets_dir ) {
  ...
  if (!(Test-Path $crops_dir )) {
    Write-Warning "Skipping analyze_crop.ijm. Could not find crops dir for analyze_crop.ijm:  $crops_dir  "
  } else {
